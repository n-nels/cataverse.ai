...
| # | Task | File(s) | Notes |
|---|------|---------|-------|
| 5.1 | Create `src/experiments/adsorption.py` (new version, lives alongside old one during transition — use a `v2_` prefix or put in a `v2/` subdirectory temporarily if needed to avoid collision). `AdsorptionExperiment` class. Constructor takes `session: ExperimentSession`, `devices: DeviceManager`, `gas: GasDelivery`, `temp: TemperatureController`, `spec: SpectrometerController`. Rewrite each method to use control and logging layer instead of raw device calls and inline threading. | `src/experiments/adsorption_v2.py` or `src/experiments_v2/adsorption.py` | The key outcome: experiment methods should read like protocols, not implementation details. |
| 5.2 | Create new `isotopic_exchange.py` using same approach. `IsotopicExchangeCalibration` class. | `src/experiments/isotopic_exchange_v2.py` | |
| 5.3 | Create `main_v2.py` at repo root. Build the full session: `load_config()` → `DeviceManager` → controllers → experiment. Show the equivalent of the current `main.py` workflow using the new architecture. | `main_v2.py` | Lives alongside `main.py`. |
...
- `main_v2.py` is a drop-in replacement for `main.py` (uses new architecture, same experiment workflows)
- All tests pass across the entire test suite
- Stale files removed
- `MIGRATION.md` written with hardware validation checklist
- **Old code (`core/`, `devices/`, `operations/`, old experiment files) is NOT deleted** — that happens after hardware validation
