...
class DirectoryWatchHandler(FileSystemEventHandler):
    def __init__(self, processed_files_log):
...
def voight_fit(file_path):
    ...
    save_data(new_data=df_fit_peaks_sorted, file_path=path)
    ...
    path = os.path.join(save_dir, filename)
    save_data(new_data=df_sum_areas, file_path=path)
    ...
    path = os.path.join(save_dir, filename)
    save_data(new_data=df, file_path=path)
...
def start_directory_watcher(directory_to_watch):
    log_directory = "C:\\Data\\OpusConvert_subIFG_lgRfl\\log"
    processed_files_log = os.path.join(log_directory, "processed_files.log")

    event_handler = DirectoryWatchHandler(processed_files_log)
    observer = Observer()
...
def main():
    ...
    # parser.add_argument('directory', type=str, help='The directory to watch.')

    # args = parser.parse_args()

    # if args.directory:
    #     start_directory_watcher(args.directory)
    # else:
    #     print("Please specify a directory to watch.")
    #     exit(1)

    subIFG_dir = "C:\\Data\\OpusConvert_subIFG_lgRfl\\test"
    start_directory_watcher(subIFG_dir)

if __name__ == "__main__":
    main()
    # file = 'C:\\Data\\OpusConvert_subIFG_lgRfl\\test\\20241224_120110_pd_ceo2_000-010_delta10.0032'
    # voight_fit(file)