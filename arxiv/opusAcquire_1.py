...
def DoSampleMeasurement(addParams):
    
    program_path = "C:\sftp\copybutton.bat"
    ...
    for i in range(len(delay)):
        for k in range(repeat[j]):
            n = str(m).zfill(4)
            now = datetime.now()
            meas = PipeCommand("COMMAND_LINE MeasureSample (, {EXP='" + XpmName + "', "
                    "XPP='" + XpmPath + "', NAM='" + FileName + "." + n + "',"
                    "SNM='" + SampleName + "',"
                        "PTH='" + path_OpusFiles + "'" + addParams + "});", 1
                        ).split(chr(10))

            SaveAs_lgRfl(meas[2], n)            
            SaveAs_ScSm(meas[2], n)
            
            if i == 0 and k == 0:
                UnloadFile(meas[2])
            else:
                SubtractIFG(meas[2], bckg)
                SaveAs_subIFG(meas[2], n)
                UnloadFile(meas[2])

            bckg = meas[2]
            LoadFile(bckg, n)

            dat = ReadParameter(meas[2], 'lgRefl', 'DAT')
            dat_in = datetime.strptime(dat.split()[0],"%d/%m/%Y").date()
            dat_out = dat_in.strftime("%m/%d/%Y")
            dat = datetime.strptime(dat_out, "%m/%d/%Y").date()

            tim = ReadParameter(meas[2], 'lgRefl', 'TIM')
            tim = datetime.strptime(tim.split()[0], "%H:%M:%S.%f")

            pka = ReadParameter(meas[2], 'lgRefl', 'PKA')
            pka = pka.split()[0]
            
            with open(read_params, 'a') as file:
                file.write(f'{FileName}.{n},{dat},{tim.time()},{pka}\n')
...
def main(sample_name, folder_name):
    ...
    # Define filename and collect spectra
    now = datetime.now()
...
main('NN2015_04PdCe_COAds_8', 'NN2015_04PdCe_COAds') # sample name, folder name
