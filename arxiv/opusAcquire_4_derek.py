    ...
    for step in range(1, 11):
        for i in range(step, len(files), step):
            ...
            if pair not in processed_files:
                ...
                name = f"{file_ids[0].split('\\')[-1][:-8]}_delta{step}.{file_ids[0].split('.')[-1][:-3]}"
...
def main(sample_name, folder_name, messenger):
    print("\nStart of program...")
    ...
    while True:
        messenger.SendText("message " + str(counter))
        counter += 1
        sleep (1)
...
main('NN2019_04PdCe_15', 'NN2019_04PdCe_COAds', None) # sample name, folder name
