...
def peak_fit(file):
    ...
    if 'isoX' in file:
        thread_isoX = threading.Thread(target=fit.integrate_irIsoXchg, args=(file,))
        thread_isoX.start()
    else:
        thread_peakFit = threading.Thread(target=fit.voight_fit, args=(file,))
        thread_peakFit.start()

        thread_findPeaks = threading.Thread(target=fit.peak_heights, args=(file,))
