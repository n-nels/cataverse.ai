...
def test_deliver_gas_to_manifold_executes_expected_valve_write_sequence() -> None:
    valves = MagicMock()
    valves.write.side_effect = lambda actuator_id, voltage: (
        actuator_id,
        float(voltage),
    )
    ...
    gas = GasDelivery(
        ...
        + _CFG.system.tube_50ml_volume_l,
        temperature_k=_CFG.system.manifold_temperature_k,
        gas_constant=_CFG.system.gas_constant,
    )
    ...
    assert [c.args[1] for c in write_calls] == pytest.approx(
        [1.0, 1.1, 1.2, 1.16, 1.26, 1.0]
    )
...
def test_evacuate_cell_uses_roughpump_sequence_then_opens_turbopump() -> None:
    ...
    last_write_index = max(
        i for i, c in enumerate(valves.mock_calls) if c == call.write("RoughPump", 5.0)
    )
    open_turbo_index = max(
        i for i, c in enumerate(valves.mock_calls) if c == call.open("TurboPump")
    )
