...
- [x] Correction model using compact intermediate-fit trajectories (per-experiment convergence extrapolation, `trajectory_extrapolation_model.py`; see SESSION_LOG.md 2026-09-18 later entry). Best late-stage parameter RMSE of any candidate, but worst remaining-curve RMSE; not selected under the current early-weighted selection score. Root cause of the curve/parameter mismatch not yet diagnosed.
...
| 2026-09-18 | Fixed a survivorship-bias scoring bug (Ridge's RMSE was computed on only its easiest ~54% of cutoffs) and added `gated_blend_model.py`, an elapsed-time-gated RF/current-ODE blend. | Re-run `train-sequential-model`: gated-blend beats RF-only on curve RMSE by ~19-57% mid/late but loses early; RF-only remains selected because `selection_score` only weights overall parameter RMSE plus early curve RMSE. | RF-only stays deployed; gated-blend is validated-but-unused evidence. Whether to broaden the selection score to reward mid/late performance is an open decision for the user, not yet made. |
| 2026-09-19 | Measured the accuracy ceiling: ran each validation experiment's own reference parameters through the ODE at every cutoff. | Remaining-curve RMSE is 0.033 early, 0.025 middle, 0.026 late (2,004 scored cutoffs, 39 experiments); the per-cutoff median is 0.027 and the worst single cutoff is 0.12. | The ODE reproduces these curves almost exactly when given the right parameters, and it does so equally well at every stage. The entire early/late gradient seen in every candidate is parameter-prediction error, not model-form error. Early is therefore open, not hopeless. |
| 2026-09-19 | Replaced `selection_score = pooled parameter RMSE + early curve RMSE` with the equal-weighted mean of early/middle/late remaining-curve RMSE, and added scale-normalized per-target parameter error to every candidate record. | The old score was a `q_e`/`q_inf` number (parameters differ by four orders of magnitude) plus an early-only curve term, so it never measured the stated objective in spec.md §14. The learned and baseline evaluators also aggregated separately, which is how the earlier validation/test mismatch arose; both now share one aggregation function. | `current_ode` (Baseline B) becomes the validation-selected candidate at 0.2096, ahead of `gated_blend_4` 0.2402 and `rf_only` 0.2957. Written to `artifacts/phase0/sequential_model_curve_score/` so the deployed `rf_only` artifacts and the frozen test report stay untouched. Test set NOT reopened. |
| 2026-09-19 | Added `raw_series_model.py`: one supervised regressor reading the raw measurement prefix resampled onto a fixed log-spaced absolute-time grid plus an availability mask, with constraints imposed through the fitter's own parameterization (`k_a` in log space, `k_p` as a bounded fraction of `k_a`) rather than by clipping, and scale-standardized targets. | Best parameter accuracy of any candidate: average scale-normalized RMSE 1.04 (MLP) versus 1.25 for RF-only, 1.31 for predicting the training mean, and 2.80 for the current ODE fit; on the four identifiable parameters it is 0.46 versus RF-only's 0.75. Its curve RMSE is nonetheless the worst of the serious candidates (0.383 versus `current_ode` 0.210), and worse at the median too (0.266 vs 0.217 for RF-only), so this is not outlier-driven. Stage by stage against RF-only it has better parameters and worse curves in all three stages; late-stage it matches the plain ODE fit's parameter accuracy (0.997 vs 0.975 normalized) while its curves are five times worse (0.326 vs 0.064). A six-variant regularization sweep, with the decision rule fixed beforehand, made both metrics monotonically worse, so overfitting is not the cause. | Within a stage, parameter accuracy does not produce curve accuracy: the parameter-to-curve map is non-identifiable, so a coordinate-wise parameter loss optimizes the wrong thing. The next step is to change the training objective to curve error — forecast the remaining curve and recover parameters by fitting the existing ODE to the forecast — not to tune this architecture further. Pooled cross-stage parameter comparisons must not be used as evidence here; the current ODE fit's pooled 2.80 is early-cutoff wildness, and within each stage its parameters and curves move together. |
| 2026-09-18 | Added `trajectory_extrapolation_model.py`, extrapolating each experiment's own rolling-fit trajectory (`value ~ intercept + slope/observation_count`) to the endpoint, instead of using only the current-cutoff snapshot. | Re-run `train-sequential-model`: aggregate parameter RMSE looked best of any candidate, but a read-only diagnostic found this was a `q_e`/`q_inf`-dominated artifact of unweighted RMSE across mismatched parameter scales. `k_a` clips to its `0.0` lower bound in 87% of valid predictions (early rolling fits are too noisy for a `1/n` extrapolation), which forces `k_p = 0` too and produces a flat, dead curve forecast — the actual, verified cause of the candidate's poor curve RMSE. | Not selected under the current selection score, and not a data point for the score-broadening question — this candidate's problem is a dead-rate-constant failure mode, not an early/late weighting issue. Next: extrapolate `k_p/k_a` ratio instead of raw `k_p` (bounded `[0,1]`, can't independently collapse to zero), or anchor extrapolation to the current fit as a bounded correction rather than an unconstrained intercept. |
