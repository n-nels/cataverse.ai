...
Responsibilities:
- Experiment/session metadata (`*_expParams.json`, experiment-ID naming, share-drive copies)
...
Current structure:
- `session.py` — `ExperimentSession`: metadata, naming, JSON persistence, end-of-experiment finalization
...
- `ExperimentSession` owns path construction and JSON metadata persistence. Plumbing methods that construct and start threaded loggers live here (not on the experiment class).
