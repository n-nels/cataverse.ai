...
class KineticWriter:
    ...
    def _prepare_model_rows_for_file(
        ...
        *,
        ...
        if model_key == "secondary_pfo":
            effective_p0 = self._select_secondary_p0_for_file(
                df_legacy,
                target_peak_name="monomer_sum",
                threshold_r2=0.96,
                user_p0=p0,
    ...
    def _select_secondary_p0_for_file(
        self,
        df_prepared: pd.DataFrame,
        *,
        target_peak_name: str = "monomer_sum",
        threshold_r2: float = 0.96,
        user_p0: list[float] | None = None,
        min_points: int = 4,
    ) -> list[float]:
        """Choose a file-level secondary_pfo p0 based on monomer_sum full-series fit.

        Search path (sequential, best-carry-forward):
        1) k_a in {current, 3e-5, 6e-4}
        2) q_e in {current, q_guess/2}
        3) k_s in {current, 9e-5, 1e-5}
        4) k_p_ratio in {current, 0.1, 1.0}
        5) q_inf in {current, 0.5}

        Returns best candidate seen so far; early-exits if threshold reached.
        """
        peak_rows = cast(
            pd.DataFrame,
            df_prepared[df_prepared["Peak_Name"] == target_peak_name].copy(),
        )
        if peak_rows.empty:
            return user_p0 if user_p0 is not None else [3e-4, 1.0, 5e-5, 0.5, 0.0]

        peak_rows = peak_rows.sort_values(by="Time (s)").reset_index(drop=True)
        time_s = peak_rows["Time (s)"].to_numpy(dtype=float)
        intensity = peak_rows["Cumulative_Peak_Area"].to_numpy(dtype=float)
        ...
        *,
        ...
        for group_key, group_raw in df.groupby("Peak_Name"):
            ...
            if mode == "full_series":
                ...
                popt, std_errors, r_squared, rmse = spec.fit_fn(time_s, intensity, p0)
    ...
    def remove_legacy_pfo_columns_file(
        ...
        *,
        ...
        carbonyl_peak_area_path = Path(carbonyl_peak_area_path)
        df_legacy = pd.read_csv(carbonyl_peak_area_path)
        cleaned_df, dropped = self.utils.drop_columns_with_prefixes(df_legacy, prefixes)
        if dropped:
            LOGGER.info(
                "Dropped %s columns from %s", len(dropped), carbonyl_peak_area_path
            )
        else:
            LOGGER.info("No prefixed columns found in %s", carbonyl_peak_area_path)
        ...
        *,
        ...
        for csv_file in csv_files:
            try:
                ...
                LOGGER.warning("Failed PFO column cleanup for %s: %s", csv_file, exc)
        return outputs
