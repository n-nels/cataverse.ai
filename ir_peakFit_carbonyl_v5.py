...
def voight_fit(file_path):
    ...
    def add_params(file_path, params, peak, peak_name):
        ...
        if (peak <= 2201) and (peak >= 2191):
            params.add(f'center_{peak_name}', value=peak, min=peak - 2,
                    max=peak + 2)
    ...
    def find_fsd_peaks():
        ...
        fsd_baseline = pybaselines.classification.std_distribution(fsd, half_window=10,
    ...
    def pfo_fit(df_cpa):
        ...
        def fit_and_evaluate(x, y, func, p0=None):
            with warnings.catch_warnings():
                ...
                try:
                    ...
                    return np.full(popt_length, np.nan), np.full(popt_length, np.nan),\
                        np.nan, np.nan
        ...
        for peak_name, group in df_cpa.groupby('Peak_Name'):
            ...
            for i in range(5, len(group)):
                ...
                popt_pfo_decay, std_errors_pfo_decay, r_squared_pfo_decay, \
                    rmse_pfo_decay = fit_and_evaluate(x, y, pfo_decay, p0=[1e-4, 1e-6, 0.1]) # for cumulative_peak_area
                
                ka_pfo_decay, kd_pfo_decay, q_pfo_decay = popt_pfo_decay
                ka_stderr_pfo_decay, kd_stderr_pfo_decay, q_stderr_pfo_decay = std_errors_pfo_decay
        ...
        if dfs:
            ...
            df_cpa_pfo = df_cpa_pfo.sort_values(['Peak_Name', 'Delta_Group']).reset_index(drop=True)
            return df_cpa_pfo
    ...
    def import_calibration_data():
        ...
        if folder_name == 'nn1120-3_pd_ceo2_000':
            ...
            skip_files = ['allData', '000-005', '000-007', '000-009', '000-010', '000-012']
            try:
                for file in glob.glob(calibration_dir + '*'):
                    if any(string in file for string in skip_files):
                        continue
    ...
    def calibration_statistics():
        ...
        pred_stderr = see * np.sqrt(1 + 1/len(x1) + (
            x1 - np.mean(x1))**2 / np.sum((x1 - np.mean(x1))**2))

        # Calculate the prediction interval
        pred_interval = t_critical * see * np.sqrt(1 + 1/len(x1) + (
            x1 - np.mean(x1))**2 / np.sum((x1 - np.mean(x1))**2))
    ...
    def save_peak_parameters():
        ...
        try:
            ...
            df_fit_peaks['PdCO_mol_stderr'] = see * np.sqrt(1 + 1/len(x1) + (
                    df_fit_peaks['Peak_Area'] - np.mean(x1))**2 / np.sum((x1 - np.mean(x1))**2))
        except Exception as e:
            pass
    ...
    def save_peak_area_versus_time(path):
        ...
        for (peak_name, delta_group), group in df.groupby(['Peak_Name', 'Delta_Group']):
            if delta_group == 'delta1':
                cumulative_area_delta1 = group[group['Delta_Group'] == 'delta1']['Peak_Area'].iloc[:2].sum()
                cumulative_integral_delta1 = group[group['Delta_Group'] == 'delta1']['Data_Integral'].iloc[:2].sum()
                time_sec_delta1 = group[group['Delta_Group'] == 'delta1']['Time_Delta (s)'].iloc[:2].sum()
                try:               
                    cumulative_PdCO_delta1 = group[group['Delta_Group'] == 'delta1']['PdCO_mol'].iloc[:2].sum()
                    cumulative_stderr_sumSqrs_delta1 = (group[group['Delta_Group'] == 'delta1']['PdCO_mol_stderr'].iloc[:2]**2).sum()
        ...
        if all(col in df_sum_areas.columns for col in ['Delta_Group', 'Time (s)', 'Peak_Name', 'Cumulative_Peak_Area']):
            ...
            df_sum_areas = pd.concat([df_sum_areas, pd.DataFrame(monomer_rows)], ignore_index=True)
    ...
    # 13CO
    peakList_core = [2196, 2176, 2156, 2146, 2136, 2125, 2113, 2103, 2093, 2073, 2062, 2050,
                 2040, 2030, 2015, 2000, 1988, 1975]
    ...
    def save_peak_area_versus_time(path):
        ...
        for (peak_name, delta_group), group in df.groupby([
            ...
            if delta_group == 'delta1':
                ...
                delta1_PdCO = group[group[
                    'Delta_Group'] == 'delta1']['PdCO_mol'].iloc[:2].sum()
                delta1_stderr_sumSqrs = (group[group[
                    'Delta_Group'] == 'delta1']['PdCO_mol_stderr'].iloc[:2]**2).sum()
        ...
        df_sum_areas = pd.DataFrame(sum_areas_list)
        filename = file_name.replace('CarbonylPeakFitParams.csv', 'CarbonylPeakArea.csv')
        path = os.path.join(save_dir, filename)
        df_sum_areas.to_csv(path, index=False)
...
def kinetic_fit(file_path):
    ...
    for peak_name, group in df_cpa.groupby('Peak_Name'):
        ...
        popt_pfo_decay, std_errors_pfo_decay, r_squared_pfo_decay, \
            rmse_pfo_decay = fit_and_evaluate(
                x, y, pfo_decay, p0=[1e-6, 1e-8, 1e-9])
        ka_pfo_decay, kd_pfo_decay, q_pfo_decay = popt_pfo_decay
...
def pfo_fit_by_row(file_path):
    ...
    try:
        df_cpa = df_cpa.drop(columns=['ka_s-1', 'ka_stderr', 'kd_s-1', 'kd_stderr',
            'qe_mol', 'qe_stderr', 'r^2', 'rmse'])
    except KeyError:
        pass
    dfs = []
    ...
    for peak_name, group in df_cpa.groupby('Peak_Name'):
        ...
        for i in range(2, len(group)):
            ...
            popt_pfo_decay, std_errors_pfo_decay, r_squared_pfo_decay, \
                rmse_pfo_decay = fit_and_evaluate(x, y, pfo_decay, p0=[1e-4, 1e-6, 0.1]) 
            ...
            result = {
                'Peak_Name': peak_name,
                'Time (s)': current_row['Time (s)'],
                'original_index': current_row['original_index'],  # Use the index from the sorted df
                'ka_s-1': ka_pfo_decay,
                'ka_stderr': ka_stderr_pfo_decay,
                'kd_s-1': kd_pfo_decay,
                'kd_stderr': kd_stderr_pfo_decay,
...
def peak_heights(file_path):
    ...
    def import_time():
        ...
        exp_params['iterator'] = exp_params['file_directory'].apply(lambda x: x.split('.')[-1])
    ...
    def import_data(dir):
        ...
        arr = df.values
        hydroxyl = (arr[:, 0] >= 3000) & (arr[:, 0] <= 3900) # change to 2200 and reprocess
    ...
    def peak_find(arr, prominence, peak_height, peaks_heights):
        ...
        baseline = pybaselines.classification.std_distribution(y, half_window=10,
                    interp_half_window=5, fill_half_window=6, num_std=1.1,
                    smooth_half_window=None, weights=None)[0]
        y_bs = y - baseline

        peaks, properties = find_peaks(y_bs, prominence=prominence,
                                        height=peak_height)
    ...
    fsd_peaks_heights = []
    peak_find(arr=arr_fsd_hydroxyl, prominence=0.0001, peak_height=0.01, peaks_heights=fsd_peaks_heights)
    peak_find(arr=arr_fsd_carbonate, prominence=0.001, peak_height=0.01, peaks_heights=fsd_peaks_heights)
    save_peak_heights(data=fsd_peaks_heights, dtype='fsd')

    lgrfl_peaks_heights = []
    peak_find(arr=arr_lgRfl_hydroxyl, prominence=0.0002, peak_height=0.001, peaks_heights=lgrfl_peaks_heights)
    peak_find(arr=arr_lgRfl_carbonate, prominence=0.0002, peak_height=0.001, peaks_heights=lgrfl_peaks_heights)
    save_peak_heights(data=lgrfl_peaks_heights, dtype='lgrfl')

    subifg_peaks_heights = []
    dtype = 'subifg'
    peak_find(arr=arr_subifg_hydroxyl, prominence=0.0001, peak_height=0.001, peaks_heights=subifg_peaks_heights)
    peak_find(arr=arr_subifg_carbonate, prominence=0.0001, peak_height=0.001, peaks_heights=subifg_peaks_heights)
...
def integrate_msIsoXchg(folder_name, files_isoX):
    ...
    if dict_msCalib is None:
        return
    df = integrate_isoX_data(dict_msCalib, mz2928_high, mz2928_low, mz2945_const, ddIG_filter)
    if df.empty:
        return
    save_data(file_prefix, df, folder_name)
...
def generate_calibCurve(folder_name):
    ...
    def ms_integral_to_moles()->None:
        ...
        df_mz29_isoX['co_moles'] = df_mz29_isoX['integral_vals'] * mz29_mol13CO_slope + mz29_mol13CO_intercept
        return None
...
def plot_spectrum_fit(file_path):
    ...
    try:
        df_fitParams = pd.read_csv(f'{fitParams_dir}\\{file_name}_CarbonylPeakFitParams.csv')
    except FileNotFoundError:
        print(f"File {file_name}_CarbonylPeakFitParams.csv not found in {fitParams_dir}.")
        return
    ...
    delta_groups = {}
    subIfg_files = sorted(glob.glob(f"{root_dir}\\OpusConvert_subIFG_lgRfl\\{folder_name}\\{file_name}*"))
    for file in subIfg_files:
        delta_file = file.split("_")[-1].split('.')[0]
        if delta_file not in ['delta5', 'delta6', 'delta7', 'delta8', 'delta9', 'delta10']:
            continue
    ...
    for delta_file, files in delta_groups.items():
        ...
        for file in files:
            ...
            arr = df.loc[(round(df[0], 4) >= x_limits[1]) & (round(df[0], 4) <= x_limits[0])].values[:, 1]
    ...
    delta_group_fit = {}
    df_fitParams = df_fitParams.dropna(subset=['Y0', 'Amplitude', 'Center', 'Sigma', 'Gamma'])
    for delta_group, group in df_fitParams.groupby('Delta_Group'):
        y = np.zeros_like(x)
        for index, row in group.iterrows():
            y += voigt_model(x, row['Y0'], row[
                'Amplitude'], row['Center'], row['Sigma'], row['Gamma'])
        delta_group_fit[delta_group] = y

    # Reconstruct the individual Voigt peak for each group
    peaks_data = {}
    for (delta_group, peak_name), group in df_fitParams.groupby(['Delta_Group', 'Peak_Name']):
    ...
    peaks = ['Peak_2093', 'Peak_2103', 'Peak_2113', 'Peak_2125', 'Peak_2136', 'Peak_2146', 'Peak_2156']
    for delta_group, arr in delta_group_data.items():
        fig, ax = plt.subplots(figsize=(4, 4))
        ax.plot(x, arr, linestyle='-', color='black', label=f'{delta_group} fit')
        ax.plot(x, delta_group_fit[delta_group], linestyle='-', color='red', linewidth=1, label=f'{delta_group} data')
        ax.plot(x, delta_group_residual[delta_group], linestyle='-', color='blue', linewidth=1, label=f'{delta_group} residual')
        for peak in peaks:
            ax.plot(x, peaks_data[delta_group][peak], linestyle='--', linewidth=1, label=peak)
        ...
        plt.savefig(f'C:\\Figures\\{folder_name}\\{file_name}_{delta_group}.tiff', dpi=800, bbox_inches='tight')
        plt.close()
...
if __name__ == "__main__":
    ...
    def process_all_monomer_sum_and_fit(
        ...
        for fname in os.listdir(src_folder):
            if fname.endswith("_CarbonylPeakArea.csv"):
                ...
                if not all(col in df.columns for col in ['Delta_Group', 'Time (s)', 'Peak_Name', 'Cumulative_Peak_Area']):
                    continue

                # Group by Delta_Group and Time (s) for monomer peaks
                grouped = df[df['Peak_Name'].isin(monomer_peaks)].groupby(['Delta_Group', 'Time (s)'])
                monomer_rows = []
                for (delta_group, time_s), group in grouped:
                    summed_area = group['Cumulative_Peak_Area'].sum()
    ...
    def process_all_integrateIsoX():
        ...
        processed_files = set("_".join(file_name.split("_")[:-1]) for file_name in os.listdir(save_dir))
